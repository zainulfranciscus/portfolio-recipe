...
public class RecipeTest extends AbstractServiceImplTest {
	...
	public void testRecipeName() {
		...
		assertEquals("Please specify the name of this recipe.", violations.iterator().next().getMessage());
	}
	...
	public void testRecipeAuthor() {
		...
		assertNotNull(violations);
		assertEquals(1, violations.size());
		assertEquals("Please specify the publisher of this recipe.", violations.iterator().next().getMessage());
	}
